...
function AdminDashboard() {
  ...
  const TABS = [
    ...
    {
      key: "requests",
      label: `📝 Requests${stats?.pendingRequests > 0 ? ` (${stats.pendingRequests})` : ""}`,
    },
  ...
  const dm = {
    ...
    cardSimple: dark ? "bg-[#1a1d27] rounded-2xl shadow-sm p-4 flex items-center justify-between border border-[#2e3347]" : "bg-white rounded-2xl shadow-sm p-4 flex items-center justify-between",
    text1:      dark ? "text-gray-100" : "text-gray-800",
    text2:      dark ? "text-gray-400" : "text-gray-500",
    text3:      dark ? "text-gray-500" : "text-gray-400",
    inputBg:    dark ? "#22263a" : "#f9fafb",
    inputBorder:dark ? "#2e3347" : "#e5e7eb",
    pendingBg:  dark ? "#2a2310" : "#fffbeb",
    ...
    <div className={dm.page} data-admin-theme={dark ? "dark" : "light"}>
      ...
      <div className={dm.header}>
        <div>
          ...
          <p className={dm.headerSub}>
            SafarSetu Administration · {user?.name}
          </p>
        </div>
        ...
        <div className="flex gap-1 overflow-x-auto">
          {TABS.map((t) => (
            <button
              ...
              className={dm.tabBtn(activeTab === t.key)}
            >
              {t.label}
            </button>
          ))}
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
                    ...
                    {stats.recentRequests.map((r) => (
                      <div
                        ...
                        style={{
                          ...
                          marginBottom: 8, gap: 8, border: "1px solid var(--border)",
                        }}
                      >
                        <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ background: "var(--surface)", borderRadius: 16, padding: "20px 20px 12px", boxShadow: "var(--shadow)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: "var(--text-muted)", background: "var(--surface2)", padding: "2px 8px", borderRadius: 8 }}>
                      Live
                    </span>
                  </div>
                  ...
                  {buildActivityFeed().length === 0 && (
                    <p style={{ fontSize: 12, color: "var(--text-muted)", textAlign: "center", marginTop: 8 }}>
                      Tip: switch to Users, Hotels or Bookings tab to load data
                    </p>
                  )}
                </div>
              </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <p className={`font-bold text-sm sm:text-base ${dm.text1}`}>
                            {r.type.replace(/_/g, " ").toUpperCase()}
                          </p>
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${dm.statusBadge(r.status)}`}>
                            {r.status}
                          </span>
                        </div>
                        ...
                        {r.details && Object.keys(r.details).length > 0 && (
                          <div className={`mt-2 ${dark ? "bg-[#22263a]" : "bg-gray-50"} rounded-xl p-3`}>
                            ...
                            {Object.entries(r.details).map(([k, v]) => (
                              <p key={k} className={`text-xs break-words ${dm.text2}`}>
                                {k}: <span className="font-medium">{String(v)}</span>
                              </p>
                            ))}
                          </div>
                        )}
                        <p className={`text-xs mt-2 ${dm.text3}`}>
        ...
        {/* ══ Analytics Tab ═════════════════════════════════════════ */}
        {activeTab === "analytics" &&
          (tabLoading ? <AnalyticsSkeleton /> : <HotelAnalytics dark={dark} />)}
      </div>
    </div>
  );
}
